...
static void expect_sym(char c)
{
    ...
    {
        compiler_error(current_process, "Expecting symbol %c however somethings else was provided\n", c);
    }
}
...
void parser_datatype_int_type_and_size_for_primitive(struct token* datatype_token, struct token* datatype_secondary_token, struct datatype* datatype_out);
void parser_datatype_adjust_size_for_secondary(struct datatype* datatype, struct token* datatype_secondary_token)
{
    ...
    parser_datatype_int_type_and_size_for_primitive(datatype_secondary_token, NULL, secondary_data_type);
...
void parser_datatype_int_type_and_size_for_primitive(struct token* datatype_token, struct token* datatype_secondary_token, struct datatype* datatype_out)
{
    ...
    {
        compiler_error(current_process, "You are not allowed for a secondary datatype here for the given datatype %s,\n", datatype_token->sval);
    }
...
void parser_datatype_init_type_and_size(struct token* datatype_token, struct token* datatype_secondary_token, struct datatype* datatype_out, int pointer_depth, int expected_type)
{
    ...
    switch(expected_type)
    {
        case DATA_TYPE_EXPECT_PRIMITIVE:
            parser_datatype_int_type_and_size_for_primitive(datatype_token, datatype_secondary_token, datatype_out);
            break;
...
void parser_datatype_init(struct token* datatype_token, struct token* datatype_secondary_token, struct datatype* datatype_out, int pointer_depth, int expected_type)
{
    ...
    {
        compiler_warning(current_process, "The compiler does not support 64 bit longs, therefore your long long is defaulting to 32 bits\n");
        datatype_out->size = DATA_SIZE_DWORD;
    }
}
...
void parser_append_size_for_node_struct_union(struct history* history, size_t* _variable_size, struct node* node)
{
    *_variable_size += variable_size(node);
    ...
    struct node* largest_var_node = vararible_struct_or_union_body_node(node)->body.largest_var_node;
...
void parser_finialize_body(struct history* history, struct node* body_node, struct vector* body_vec, size_t* variable_size, struct node* largest_align_eligible_var_node, struct node* largest_possible_var_node )
{
...
void parse_body_single_statement(size_t* variable_size, struct vector* body_vec, struct history* history)
{
    ...
    parser_finialize_body(history, body_node, body_vec, variable_size, largest_var_node, largest_var_node);
    parser_current_body = body_node->binded.owner;
    node_push(body_node);
}

/**
 * @param variable_size Set to the sum of all variables encountered in the parsed body
 * @param history
*/
